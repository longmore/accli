...
module.exports = {
    ...
    async get() {
        ...
        if (/\.git$/.test(url)) {
            ...
            if (fs.existsSync(`../${gitName}/index.js`)) {
                content = require(`../${gitName}/index.js`);
            }
            if (fs.existsSync(`../${gitName}/index.json`)) {
                content = require(`../${gitName}/index.js`);
            }
            if (fs.existsSync(`../${gitName}/config.js`)) {
                content = require(`../${gitName}/config.js`);
            }
            if (fs.existsSync(`../${gitName}/config.json`)) {
                content = require(`../${gitName}/config.json`);
            }
            await rimraf(gitName);
        ...
        // .js/.json文件方式下载
            ...
            // 支持4个文件读取配置 index.js index.json config.js config.json
            if (fs.existsSync(`../${gitName}/${filePath}`)) {
                content = require(`../${gitName}/${filePath}`);
            }
